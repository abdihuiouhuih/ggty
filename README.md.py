...
# --- إعدادات الصفحة ---
st.set_page_config(
    page_title="منصة الإنجاز الذكي 2026",
    layout="wide",
...
def reset_ai_messages():
    st.session_state.ai_messages = [
        {
            "role": "assistant",
            "content": "يا هلا والله! أنا مساعدك في «منصة التطوير الذاتي الشاملة». سولف معي براحتك، محتار بتخصص؟ تبي تنزل وزنك؟ ولا تبي تكسر عادة سيئة؟ اعتبرني صديقك وفضفض لي، وش شاغل بالك اليوم؟"
        }
    ]
...
if st.session_state.current_page == 'home':
    ...
    st.markdown(
        "<h1 style='text-align: center; color: #ffffff;'>🚀 منصة التطوير الذاتي الشاملة</h1>",
        unsafe_allow_html=True
    )

    st.markdown(
        "<p style='text-align: center; color: #768390;'>نظام ذكي تفاعلي لتحسين جودة حياتك اليومية ومساعدتك في كل استفساراتك</p>",
        unsafe_allow_html=True
    )
    ...
    with col1:
        ...
        st.markdown(
            '<div class="card"><h2>🏋️ التحدي الرياضي</h2><p>حساب سعرات متطور ونظام نصائح بدنية متغيرة</p></div>',
            unsafe_allow_html=True
        )
        ...
        st.markdown(
            '<div class="card"><h2>🚫 تحدي العادات</h2><p>استراتيجيات نفسية وعملية لترك العادات السلبية</p></div>',
            unsafe_allow_html=True
        )
    ...
    with col2:
        ...
        st.markdown(
            '<div class="card"><h2>📚 التحدي الدراسي</h2><p>خطط ذكية ونصائح تخصصية لـ 12 مساراً أكاديمياً</p></div>',
            unsafe_allow_html=True
        )
        ...
        st.markdown(
            '<div class="card"><h2>🤖 منصة التطوير الذاتي الشاملة</h2><p>مساعد ذكي عفوي يرد على كافة استفساراتك ويشرح لك محتويات وأقسام الموقع فوراً</p></div>',
            unsafe_allow_html=True
        )
    ...
    # =====================================================
    # 🔐 إثبات ملكية Google
    # موجود أسفل الأقسام الأربعة في الصفحة الرئيسية
    # =====================================================
...
# =========================================================
    ...
    if st.button("📊 توليد التقرير البدني والنصائح"):
        ...
        st.success(
            ...
            f"احتياجك لهدفك: "
            f"{int(tdee-500 if 'خسارة' in goal else tdee+400 if 'بناء' in goal else tdee)} سعرة."
        )
        ...
        selected_advices = random.sample(
            fitness_advices,
            3
        )
...
# =========================================================
    ...
    with col_h1:
        ...
        habit_name = st.text_input(
            "ما هي العادة التي تود كسرها؟ (مثلاً: التدخين، السهر، السكريات)"
        )
    ...
    if st.button("🚀 تحليل العادة ووضع خطة"):
        ...
        st.info(
            f"📍 **نصيحة للمحفز ({trigger}):** {habit_tips[trigger]}"
        )
...
# =========================================================
    ...
    study_data = {
        ...
        "هندسة الشبكات": [
            "تخصص في محاكاة GNS3 و EVEng.",
            "احصل على شهادة CCNA قبل التخرج.",
            "افهم OSI Model كأنه اسمك."
        ],

        "الأمن السيبراني": [
            "تعلم أساسيات Linux (Kali/Parrot).",
            "مارس تحديات CTF (Capture The Flag) أسبوعياً.",
            "شهادة Security+ هي بوابتك الأولى."
        ],

        "الذكاء الاصطناعي": [
            "أتقن الرياضيات (Linear Algebra).",
            "تعلم مكتبات Pandas و Scikit-learn.",
            "ابنِ مشاريع تنبؤية بسيطة ببيانات حقيقية."
        ],

        "علوم الحاسب": [
            "ركز على هياكل البيانات (Data Structures).",
            "حل مشكلات البرمجة في LeetCode.",
            "افهم كيف تعمل الذاكرة (Memory Management)."
        ],

        "الطب": [
            "استخدم Anki للحفظ طويل الأمد.",
            "اربط المعلومة النظرية بالحالة السريرية.",
            "ركز على مراجعة علم الأمراض (Pathology) باستمرار."
        ],

        "الهندسة الميكانيكية": [
            "أتقن برامج الـ CAD مثل SolidWorks.",
            "افهم الديناميكا الحرارية بعمق تطبيق عملي.",
            "تابع مشاريع التصنيع الحديثة والـ 3D Printing."
        ],

        "إدارة الأعمال": [
            "تعلم مهارات تحليل البيانات بـ Excel و Power BI.",
            "اقرأ في القيادة وعلم النفس المؤسسي.",
            "افهم استراتيجيات التسويق الرقمي."
        ],

        "المحاسبة": [
            "افهم الـ IFRS (المعايير الدولية).",
            "تدرب على برامج مثل SAP أو Oracle.",
            "دقة الأرقام أهم من سرعتك في الحل."
        ],

        "القانون": [
            "درب نفسك على الصياغة القانونية الرصينة.",
            "تابع أحكام المحاكم العليا لفهم روح القانون.",
            "مارس المحاكم الصورية (Moot Court)."
        ],

        "التمريض": [
            "الجانب الإنساني والعناية بالمرضى يوازي العلم الطبي.",
            "أتقن مهارات الطوارئ والإنعاش (BLS).",
            "الدقة في قياس المؤشرات الحيوية تنقذ الأرواح."
        ],

        "الهندسة الكهربائية": [
            "ركز على أنظمة الطاقة المتجددة والتحكم.",
            "أتقن MATLAB للمحاكاة.",
            "افهم الدوائر المتكاملة والأنظمة المدمجة."
        ]
    }
    ...
    st.subheader(
        "💡 نصائح دراسية عامة (تتغير عند كل دخول):"
    )
